...
INSERT INTO `cart` VALUES (1,1,'2025-12-15 08:30:52');
...
INSERT INTO `cart_items` VALUES (15,1,240,2),(16,1,236,1),(17,1,238,1),(18,1,4,1);
...
INSERT INTO `szin` VALUES (9,'Barna'),(8,'B├ęzs'),(2,'Feh├ęr'),(1,'Fekete'),(4,'K├ęk'),(10,'Lila'),(11,'Narancss├írga'),(5,'Piros'),(12,'R├│zsasz├şn'),(7,'S├írga'),(3,'Sz├╝rke'),(6,'Z├Âld');
...
CREATE TABLE `termekek` (
  ...
  CONSTRAINT `FK_termekek_marka_id` FOREIGN KEY (`marka_id`) REFERENCES `marka` (`id`),
...
INSERT INTO `users` VALUES (1,'testuser2','testuser@gmail.com','$2b$10$jnX/6D0YuHljkd6m9.vAkuHJ7oPV36izI6HPi4ZbDiGMsVXPcPXKa','user'),(2,'testuser','testuser2@gmail.com','$2b$10$DU0uyM1dawLywXdDPpLRR.nqDyOy0pb0ndaQ1wPBlOD.9EfPY4Rr2','user'),(3,'bende','busibendi11@gmail.com','$2b$10$.oNf7unn8Zluz90cjE2O0.FFPnuAJ4j2oZYymORn4wJ2/G0r6ygB6','user'),(4,'king','king@gmail.com','$2b$10$EV6vBWJhi2cn1KOVn.DYousLlpteDjb/Y6pGbCDKTrAWeUt2eICTG','user'),(5,'kiraj','kiraj@gmail.com','$2b$10$Qd0zVKFzIUn3T5ogaFO0duCHbcF2V5VkHPRgoXjhAhcK9yCJ0v3o2','user');
...
INSERT INTO `vevo` VALUES (1,'32926113-1-24','Kiss P├íl','5400, Mez┼Ĺt├║r Pet┼Ĺfi t├ęr 1'),(2,'12345678-1-42','Kov├ícs B├ęla','Budapest, F┼Ĺ utca 1.'),(3,'87654321-1-33','Szab├│ Anna','Debrecen, Kossuth t├ęr 5.'),(4,'11112222-1-11','T├│th G├íbor','Szeged, Tavasz utca 10.'),(5,'22221111-1-22','Nagy M├írta','P├ęcs, Ny├ír utca 8.'),(6,'33334444-1-33','Fekete L├íszl├│','Gy┼Ĺr, ┼Ész utca 4.');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `gyak3` AS select `v`.`nev` AS `Vasarlo_Nev`,`s`.`id` AS `Szamlaszam`,sum((`t`.`ar_usd` * `st`.`mennyiseg`)) AS `Szamla_Osszeg_USD` from (((`szamla` `s` join `vevo` `v` on((`s`.`vevo_id` = `v`.`id`))) join `szamla_termek` `st` on((`s`.`id` = `st`.`szamla_id`))) join `termekek` `t` on((`st`.`termek_id` = `t`.`id`))) where (`v`.`nev` = 'Kov├ícs B├ęla') group by `s`.`id`,`v`.`nev` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view1` AS select `termekek`.`id` AS `termek_id`,`termekek`.`nev` AS `N├ęv`,`termekek`.`ar_usd` AS `├ür(usd)`,`tipus`.`tipus` AS `T├şpus`,`szin`.`szin` AS `Sz├şn`,`meret`.`meret` AS `M├ęret`,`elerhetoseg`.`statusz` AS `St├ítusz`,`marka`.`markanev` AS `M├írka`,`termekek`.`kep_id` AS `kep_id` from (((((`termekek` join `tipus` on((`termekek`.`tipus_id` = `tipus`.`id`))) join `szin` on((`termekek`.`szin_id` = `szin`.`id`))) join `meret` on((`termekek`.`meret_id` = `meret`.`id`))) join `elerhetoseg` on((`termekek`.`elerhetoseg_id` = `elerhetoseg`.`id`))) join `marka` on((`termekek`.`marka_id` = `marka`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-01-12  8:36:13
